...
def wrap_gen_captcha_text_and_image(path=None):
    '''
    返回特定shape图片
    :param shape:
    :return:
    ...
    shape = (30, 100, 1)
    while True:
        t, im = gen_captcha_text_and_image(path)
        if im.shape == shape:
            return t, im


def next_batch(batch_count=60, width=CAPTCHA_WIDTH, height=CAPTCHA_HEIGHT,path=None):
    '''
    获取训练图片组
    :param batch_count:
    :param width:
    ...
    for i in range(batch_count):
        text, image = wrap_gen_captcha_text_and_image(path)
